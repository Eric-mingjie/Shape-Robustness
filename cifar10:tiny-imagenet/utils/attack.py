...
# =======
# from .canny import canny
# >>>>>>> aed7c7e39c3c957fa4b4d807eb678c16f2dfe007
import numpy as np
...
def test_canny_on_edge(net, net_canny, eps, testloader, iteration=1, step_size=0.01, sigma=2, high_threshold=None, low_threshold=None, thres=0, path='.'):
    ...
    for index, data in enumerate(testloader):
        if index == 100:
            break
        images, labels, images_edge = data[0], data[1], data[-1]
        images = images.cuda()
        labels = labels.cuda()
        images_edge = images_edge.cuda()
        ...
        for _ in range(100):
            ...
            # outputs = net(Variable(torch.cat([img_edge, img_edge, img_edge], 1)))
