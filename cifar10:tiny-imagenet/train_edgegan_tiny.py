...
parser.add_argument('--spsa_samples', default=64, type=int)
parser.add_argument('--spsa_iters', default=1, type=int)
...
class model_all(nn.Module):
    def __init__(self, net, G, sigma, high_threshold=0.2, low_threshold=0.1, thres=0.):
    ...
    def forward(self, images):
        ...
        images = self.generator(z, img_edge)
...
def test_gen_adv(net, G, testloader, sigma=2, high_threshold=0.2, low_threshold=0.1):
    correct = 0
    total = 0
    net.eval()
    two_nets = cnnedge.InterpNets_EdgeGAN(edgenet, G, net, '3', '4')
    two_nets.eval()
    attack = LinfPGDAttack(epsilon=8/255, alpha=2/255, k=40)

    for index, data in enumerate(testloader):
        # batch_max = 30
        # if index >1 and index % batch_max == 0:
        #     break
        images, labels = data
        images = images.cuda()
        labels = labels.cuda()
        ...
        # img_edge = img_edge.cuda()
        adv_inputs = attack(two_nets, images, labels)
        
        generated1 = edgenet(adv_inputs, mode='1').detach().cuda()
        generated2 = edgenet(adv_inputs, mode='2').detach().cuda()
        img_edge = interp_z * generated1 + (1 - interp_z) * generated2
        # img_edge = get_edge(adv_inputs, sigma=sigma, high_threshold=high_threshold, low_threshold=low_threshold)
...
def test_spsa(net, G, testloader, sigma=2, high_threshold=0.2, low_threshold=0.1):
    correct = 0
    total = 0
    net.eval()
    G.eval()
    eps = 4/255
    iteration = 8
    net_all = model_all(net, G, sigma, high_threshold, low_threshold, args.thres)
    net_all.eval()
    # spsa_attack = LinfSPSAAttack(net_all, eps, nb_iter=iteration, nb_sample=2048, max_batch_size=64, clip_min=-1.0, clip_max=1.0)
    for index, data in enumerate(testloader):
        # batch_max = 100
        # if index > 0 and index % batch_max == 0:
        #     break
        images, labels = data
        images = images.cuda()
        labels = labels.cuda()

        adv_inputs = spsa(net_all, images, eps, iteration, clip_min=-1, clip_max=1, spsa_samples=args.spsa_samples, spsa_iters=args.spsa_iters)
        interp_z = torch.zeros_like(adv_inputs)[:, 0:1, :, :].uniform_(0, 1).cuda()
        generated1 = edgenet(adv_inputs, mode='1').detach().cuda()
        generated2 = edgenet(adv_inputs, mode='2').detach().cuda()
        img_edge = interp_z * generated1 + (1 - interp_z) * generated2
        # img_edge = get_edge(adv_inputs, sigma=sigma, high_threshold=high_threshold, low_threshold=low_threshold)
        ...
        # img_edge = get_edge(images, sigma=sigma, high_threshold=high_threshold, low_threshold=low_threshold)
        # img_edge = img_edge.cuda()
        # images = spsa(net_all, images, eps, iteration, clip_min=-1, clip_max=1, spsa_samples=args.spsa_samples, spsa_iters=args.spsa_iters)
        outputs = net(Variable(images))
        _, predicted = torch.max(outputs.data, 1)
        total += labels.size(0)
        correct += (predicted == labels).sum()
...
#     G.load_state_dict('./checkpoint/cifar10/edgegan/alpha_1_beta_1.0_sigma_1.0_high_0.2_low_0.1_D_1/')
...
# D.load_state_dict(torch.load('./logs2/gan/tiny_64/adv_gandiscriminator_param.pkl'))
# Adam optimizer
...
# test_gen_shift(cls_net, G, test_loader, sigma=args.sigma, high_threshold=args.high_threshold, low_threshold=args.low_threshold)
G.eval()
# test_spsa(cls_net, G, test_loader)
# fig(cls_net, G, test_loader)
# for p in np.linspace(0.5, 0.9, 5):
# test_shift_fig(cls_net, G, test_loader, type='baseline', r=91)
test_gen_adv(cls_net, G, test_loader, sigma=args.sigma, high_threshold=args.high_threshold, low_threshold=args.low_threshold)
...
for epoch in range(train_epoch):
    ...
    for x_, y_ in train_loader:
        ...
        # Cls loss
        G_result = F.interpolate(G_result, size=128, mode='bilinear')
