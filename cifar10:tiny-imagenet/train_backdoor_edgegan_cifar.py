...
def test_gen(net, G, testloader, sigma=2, high_threshold=0.2, low_threshold=0.1):
    ...
    for batch_idx in range(iter_num):
        ...
        images, labels = dataset.eval_data.get_next_batch(args.test_batch,
...
def test_gen_poi(net, G, testloader, sigma=2, high_threshold=0.2, low_threshold=0.1):
    ...
    for batch_idx in range(iter_num):
        ...
        images, labels = dataset.poisoned_eval_data.get_next_batch(args.test_batch,
                                                    multiple_passes=True)
        
        
        if clean_label > -1:
            clean_indices = np.where(labels==clean_label)[0]
            if len(clean_indices)==0: continue
            inputs = images[clean_indices]
            targets = np.repeat(target_label, len(clean_indices))
        else: 
            inputs = images
            targets = np.repeat(target_label, len(images))
        ...
        # img_edge = get_edge(inputs, sigma=sigma, high_threshold=high_threshold, low_threshold=low_threshold)
        # img_edge = img_edge.cuda()
        ...
        # if inputs.size()[0] != z.size()[0]:
        z = torch.randn(inputs.size()[0],100,1,1)
        z = z.view(-1, 100, 1, 1).cuda()
        ...
        # G_result1 = G(z_, torch.cat([generated1, generated1, generated1], 1))
        # G_result2 = G(z_, torch.cat([generated2, generated2, generated2], 1))
        # inputs = interp_z * G_result1 + (1 - interp_z) * G_result2
...
# G.load_state_dict(torch.load('./logs/cnn_gan/adv_gangenerator_param.pkl'))
...
# D.load_state_dict(torch.load('/data/common/zichao/adv-edge/logs/cnn_gan/adv_gandiscriminator_param.pkl'))
# Adam optimizer
...
for epoch in range(train_epoch):
    ...
    for batch_idx in range(iter_num):
        ...
        x_, y_ = dataset.train_data.get_next_batch(args.train_batch,
                                                    multiple_passes=True)
        x_ = torch.Tensor(x_)
        y_ = torch.Tensor(y_)
        y_ = y_.to(dtype=torch.long)
        x_ = x_.permute(0, 3, 1, 2)
        x_ = batch_transform(x_)
        count += 1
        ...
        #     save_image(x_[:16], "fig/backdoor/0/%d_vanilla.png"%epoch, nrow=4, normalize=True)
        #     save_image(G_result[:16], "fig/backdoor/0/%d.png"%epoch, nrow=4, normalize=True)
    ...
    test_gen_poi(cls_net, G, test_loader, sigma=args.sigma, high_threshold=args.high_threshold, low_threshold=args.low_threshold)
    G.train()
