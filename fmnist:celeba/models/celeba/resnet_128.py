...
__all__ = ['resnet_128', 'resnet_128_par', 'PAR']
...
class ResNet_128_PAR(nn.Module):

    def __init__(self, depth, num_classes=1000, input_channel=3):
        super(ResNet_128_PAR, self).__init__()
        # Model type specifies number of layers for CIFAR-10 model
        assert (depth - 2) % 6 == 0, 'depth should be 6n+2'
        # n = (depth - 2) // 6
...
def resnet_128_par(**kwargs):
    """
    Constructs a ResNet model.
    """
    return ResNet_128_PAR(**kwargs)


class PAR(nn.Module):
    def __init__(self):
        super(PAR, self).__init__()
        self.conv = nn.Conv2d(16, 2, 1)
        self.relu = nn.ReLU()
